...
export default function Signup({
    ...
    const signupapicall = async () => {
        ...
        if (regLat == null || regLng == null) {
            toast.error("GPS location permission is strictly required for signup. Please click 'Fetch GPS location' and grant access.")
            return
        }
        ...
        <form onSubmit={(e)=>{e.preventDefault()}} className="app-card p-8 rounded-3xl flex flex-col gap-6 w-full max-w-md">
           ...
           <div className="flex flex-col gap-2">
                <button
                  ...
                  className="app-control px-6 py-3 text-amber-500 rounded-lg text-lg cursor-pointer flex items-center justify-center gap-2"
                  onClick={handleFetchGpsLocation}
                >
                  {isFetchingGps ? "Requesting GPS Access..." : "Fetch GPS Location 📍"}
                </button>
